...
// Reusable and optimized BackgroundGrid component
const BackgroundGrid = ({ rows = 8, cols = 8, className = "" }) => {
  // Pre-generate grid cells for better performance
  const cells = useMemo(() => {
...
export default function CyborgCorpHomepage() {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!isMoving.current) {
        ...
        // Schedule cursor update using requestAnimationFrame for better performance
        requestAnimationFrame(() => {
          if (cursorRef.current) {
            // This centers the glow effect properly on the cursor
            cursorRef.current.style.transform = `translate(${e.clientX - 128}px, ${e.clientY - 128}px)`;
          }
          
          // Reset moving flag after a short delay to throttle updates
          clearTimeout(cursorTimeout.current);
          cursorTimeout.current = setTimeout(() => {
            isMoving.current = false;
          }, 20); // Limit to 50 updates per second
        });
      }
    };
  ...
  // Memoized testimonials data to prevent recreation on each render
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100 relative overflow-hidden">
      {/* Optimized cursor effect - fixed positioning with transform for better performance */}
      <div
        ref={cursorRef}
        className="fixed w-64 h-64 rounded-full bg-cyan-500/20 blur-3xl pointer-events-none z-10 opacity-50"
        style={{
          willChange: 'transform', // Hint for browser optimization
          // Remove the incorrect transform, left and top properties
          // The transform will be set by the handleMouseMove function
        }}
      />
      ...
      <header className="border-b border-gray-800 bg-black/90 backdrop-blur-sm fixed w-full z-50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          {/* CTA Button */}
          <a onClick={() => scrollToSection("contact")} className="hidden md:block bg-cyan-500 hover:bg-cyan-600 text-black px-4 py-2 rounded font-medium transition-all cursor-pointer hover:scale-105 hover:shadow-lg hover:shadow-cyan-500/20">
            Upgrade Now
          </a>
        </div>
